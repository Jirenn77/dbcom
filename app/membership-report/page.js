...
export default function MembershipExpirationReport() {
  ...
  const handleRenew = async (
    ...
    try {
      ...
      // Only allow renewal for BASIC or PRO if coverage is already consumed
      if ((type === "basic" || type === "pro") && currentMembership) {
        const remaining = parseFloat(currentMembership.remaining_balance || 0);

        if (remaining > 0) {
          toast.error(
            `Cannot renew ${type.toUpperCase()} membership while ₱${remaining.toFixed(
              2
            )} remains.`
          );
          return;
      ...
      // For BASIC and PRO, set a record expiration for tracking (not enforced)
      if (type !== "promo") {
        const today = new Date();
                      ...
                      <>
                        ...
                        <div className="flex items-center">
                          <input
                            ...
                            className="mr-2"
                          />
                          <label htmlFor="renewNoExpiration">
                            No Expiration
                          </label>
                        </div>
